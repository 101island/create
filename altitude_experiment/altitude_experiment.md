...
这个模块的目标不是“纠错”，也不是直接规定控制器输出，而是提供某个高度的稳态悬停前馈项。
...
- `target`：用目标高度；当前实验控制器默认使用这个模式，因为前馈表记录的是目标稳态悬停值
- `current`：用当前高度
- `speedTarget`：代码里也支持，但这里返回的是 `runtime.position.output`，也就是外环产生的速度目标；名字上更像实验接口，不是最直观的生产配置

最终写给执行器的是 `hover_feedforward + speed_pid_correction`，再经过限幅、变化率限制和 PDM/PWM 输出。
